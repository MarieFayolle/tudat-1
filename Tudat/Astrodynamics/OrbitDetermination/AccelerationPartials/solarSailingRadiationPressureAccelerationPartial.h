#ifndef TUDAT_SOLARSAILINGRADIATIONPRESSUREACCELERATIONPARTIAL_H
#define TUDAT_SOLARSAILINGRADIATIONPRESSUREACCELERATIONPARTIAL_H
...
//! Class to calculate the partials of the solar sailing radiation pressure acceleration w.r.t. parameters and states.
class SolarSailingRadiationPressurePartial: public AccelerationPartial
{
public:
    ...
    /*!
     * Constructor.
     * \param radiationPressureAcceleration Solar sailing radiation pressure acceleration model.
     * \param radiationPressureInterface Interface object to retrieve and/or compute the properties of
     * the solar sailing radiation pressure model.
     ...
     */
    SolarSailingRadiationPressurePartial(
            const std::shared_ptr< electro_magnetism::SolarSailAcceleration > radiationPressureAcceleration,
            const std::shared_ptr< electro_magnetism::SolarSailingRadiationPressureInterface > radiationPressureInterface,
            const std::string& acceleratedBody, const std::string& acceleratingBody ):
        AccelerationPartial( acceleratedBody, acceleratingBody, basic_astrodynamics::solar_sail_acceleration ),
            ...
            const bool addContribution = 1, const int startRow = 0, const int startColumn = 0 )
    {
        if( addContribution )
        {
            partialMatrix.block( startRow, startColumn, 3, 3 ) += currentPartialWrtVelocity_; // currentAccelerationStatePartials_.block( 0, 3, 3, 3 );
        }
        else
        {
            partialMatrix.block( startRow, startColumn, 3, 3 ) -= currentPartialWrtVelocity_; // currentAccelerationStatePartials_.block( 0, 3, 3, 3 );
            ...
            const propagators::IntegratedStateType integratedStateType )
    {
                     ...
                     integratedStateType == propagators::rotational_state ) )
        {
            throw std::runtime_error(
                        "Warning, dependency of solar sailing radiation pressure on body masses and rotational state not yet implemented" );
        }
        return 0;
    }
...
private:
    ...
    std::shared_ptr< electro_magnetism::SolarSailAcceleration > radiationPressureAcceleration_;

    //! Pointer to the panelled radiation pressure interface.
    std::shared_ptr< electro_magnetism::SolarSailingRadiationPressureInterface > radiationPressureInterface_;
    ...
    //! Current partial of acceleration w.r.t. velocity of body undergoing acceleration (equal to minus partial w.r.t.
    //! velocity of body exerting acceleration).
...
#endif // TUDAT_SOLARSAILINGRADIATIONPRESSUREACCELERATIONPARTIAL_H
